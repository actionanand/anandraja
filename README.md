...
`src/routeTree.gen.ts` is **automatically generated by TanStack Router** every time you add, rename, or delete a file inside `src/routes/`. You should **never edit this file manually** — it will be overwritten on the next run.

#### What it does

It scans all files in `src/routes/` and builds a fully type-safe **route tree** that connects every route to its parent, its path, and its TypeScript types. This is what makes the following possible in your code with zero manual wiring:

```tsx
// ✅ TypeScript error if '/candidates' doesn't exist as a route
<Link to="/candidates" />
...
- Automatically during `npm run dev` (file watcher picks up changes)
- Automatically during `npm run build` (one-time scan before bundling)
- Triggered by `TanStackRouterVite()` plugin registered in `vite.config.ts`

#### What it contains

| Export | Purpose |
|---|---|
| `routeTree` | The assembled tree passed to `createRouter()` in `src/router.tsx` |
| `FileRoutesByFullPath` | Interface mapping full path strings → route types |
| `FileRoutesByTo` | Interface used by `<Link to="...">` for type checking |
| `FileRoutesById` | Interface mapping route IDs → types |

#### Rules

- ✅ Add it to **`.gitignore`** if you prefer to generate it on CI (optional — most projects commit it)
- ✅ Add it to your **Biome / ESLint ignore list** (already handled by the `// @ts-nocheck` header it generates)
